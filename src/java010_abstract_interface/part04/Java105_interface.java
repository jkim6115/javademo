 ...
 *	2. 클래스 작성에 도움을 주기 위한 목적으로 제공되는 표준명세서.
 *	3. implement : 클래스에 인터페이스를 (구현)상속할 때 사용되는 키워드
 *	4. 인터페이스는 접근제어자로 public만 제공한다.
 *	5. 인터페이스의 추상메소드에 abstract 키워드를 명시하지 않아도 된다.
 *		자바 가상머신에서 자동으로 처리한다.
 *	6. 모든 인터페이스의 메소드는 묵시적으로 public abstract이다.
 *	7. 맴버변수는 묵시적으로 public static final이다.
 ...
 *	내부클래스		O				O
 *
 *	extends : 클래스애 클래스 상속(단일 상속), 인터페이스에 인터페이스 상속(다중상속)
 *	implement : 클래스에 인터페이스 구현(다중 구현)
 */
