...
public class Prob003_ArrayList {
	...
	private static ArrayList<SmartPhone> phoneProduct(String pathFile) {
		...
		ArrayList<SmartPhone> sp = new ArrayList<>();
		try(Scanner sc = new Scanner(new File(pathFile))) {
			while(sc.hasNextLine()) {
				String[] arr = sc.nextLine().split(":");
				SmartPhone sph = new SmartPhone();
				sph.setProductId(arr[0]);
				sph.setName(arr[1]);
				sph.setPrice(Integer.parseInt(arr[2]));
				sph.setAmount(Integer.parseInt(arr[3]));
				sph.setMaker(arr[4]);
				sp.add(sph);
			}
	...
	private static void prnDisplay(ArrayList<SmartPhone> phoneList){
		...
		Iterator<SmartPhone> sp = phoneList.iterator();
		int cnt = 1;
		while(sp.hasNext()) {
			SmartPhone shp = sp.next();
			System.out.printf("<< %d 번째 상품 >>\n", cnt++);
			System.out.println("제품아이디:"+shp.getProductId());
			System.out.println("제품명:"+shp.getName());
			System.out.println("가격:"+shp.getPrice());
			System.out.println("수량:"+shp.getAmount());
			System.out.println("제조사:"+shp.getMaker());
			System.out.println();
		}
	}//end prnDisplay( )
